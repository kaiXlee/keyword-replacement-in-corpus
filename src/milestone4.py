...
def processMultipleTexts(texts, slangDict, wordCount=float('inf'), wordsProcessedCount = 0, slangCount = 0, countractionCount= 0):
    for i, text in enumerate(texts):
        ...
        (wordsProcessedCount, contractionCount, words) = processStr2(text, contractions, wordCount, wordsProcessedCount, countractionCount)
    # return wordsProcessedCount, slangCount
    return wordsProcessedCount, contractionCount
...
def processStr2(string, contractions, wordCount=float('inf'), wordsProcessedCount=0, contractionCount=0, verbose=False):
    ...
    for i, word in enumerate(words):
        ...
        for j in range(len(hints)):
            if hints[j] in word:
                ...
                if substringShort in contractions:
                    contractionCount += 1
                    words[i] = word.replace(substringShort, " " + contractions[substringShort])
                    if verbose:
                        print(f"\"{word}\" is replaced with: \"{words[i]}\"")
                elif substringLong in contractions:
                    contractionCount += 1
                    words[i] = word.replace(substringLong, " " + contractions[substringLong])
                    if verbose:
                        print(f"\"{word}\" is replaced with: \"{words[i]}\"")
        wordsProcessedCount += 1
    return wordsProcessedCount, contractionCount, words


def tryItABunch(myFunc, texts , contractions, startN=10, endN=100, stepSize=10, numTrials=20, listMax = 10):
...
if __name__ == "__main__":
    ...
    contractions = loadSlang(os.path.join(dirname, '../dataset/contractions.txt'))
    ...
    (x, y, proccessedStr) = processStr2( string = texts[10], contractions = contractions, wordCount = 100, verbose=True)
    print("\nORIGINAL: "+texts[10])
    ...
    (nValues, timeValues) = tryItABunch(myFunc = processMultipleTexts, texts = texts, contractions = contractions, startN = 10000, endN = 48000, stepSize=1000, numTrials=50)
